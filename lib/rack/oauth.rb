  ...
  def [] key
    if not has_key?(key)
      ...
      hash.keys.each do |hash_key|
        return hash[hash_key] if hash_key.downcase == key.downcase
      end
    end
    super
  end
...
module Rack #:nodoc:
  ...
  class OAuth
    ...
    DEFAULT_OPTIONS = { :login_path    => '/auth',
    ...
    def call(env)
      ...
      when login_path then do_login(env)
      ...
      when callback_path then do_callback(env)
      else @app.call(env) end
    end

    def do_login(env)
      if Rack::OAuth.test_mode?
        set_access_token(env, OpenStruct.new(:params => {:fake => :token}))
      ...
      callback_uri = ::File.join("http://#{env['HTTP_HOST']}", callback_path)
      request = consumer.get_request_token(:oauth_callback => callback_uri)
    ...
    def do_callback(env)
      ...
      verifier = Rack::Request.new(env).params['oauth_verifier']
      set_access_token(env, request.get_access_token(:oauth_verifier => verifier))
    ...
    def verified?(env)
      [:token, :secret, :verifier].all?{|key| session(env)[key] }
    end
    ...
    def valid?
      @errors = []
      @errors << ':key option is required'    unless key
      @errors << ':secret option is required' unless secret
      @errors << ':site option is required'   unless site
      @errors.empty?
    end
    ...
    def session(env)
      raise "Rack env['rack.session'] is nil ... has a Rack::Session middleware be enabled? use :rack_session for custom key" if env[rack_session].nil?      
